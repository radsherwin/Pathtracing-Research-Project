...
Sphere sph = Sphere(light, vec3(100.,450.,100.0), 40.0); // light
...
bool scatter(HitResult hit_in, Ray r_in, vec2 seed, out ScatterResult s_out)
{
    ...
    if(m.type == mat_lambertian)
    {
        ...
        //s_out.pdf_value = cosine_pdf(seed, hit_in.p, hit_in.normal, s_out.pdf_ray);
        //s_out.out_ray = Ray(hit_in.p, random_cos_direction(hit_in.normal, seed));
...
//    hit_out.front_face = dot(r_in.direction, hit_out.normal) < 0.0;
//    hit_out.normal = hit_out.front_face ? hit_out.normal : -hit_out.normal;
...
bool hit(Ray r_in, float t_min, float t_max, out HitResult hit_out)
{
    ...
    //Sphere sph2 = Sphere(green, vec3(0,-1001.0,0), 1000.0);

    //Sphere spheres[2] = Sphere[](sph, sph2);
    ...
    Box box2 = Box(m2, white, vec3(265., 0.0, 295.), vec3(430., 330., 460.));
    Box boxes[2] = Box[](box1, box2);
//
//    Plane p1 = Plane(green, vec4(0.0, 555., 0.0, 555.), 555.0, plane_yz, false);
//    Plane p2 = Plane(red, vec4(0.0, 555., 0.0, 555.), 0.0, plane_yz, false);
//    Plane p3 = Plane(white, vec4(0.0, 555., 0.0, 555.), 0.0, plane_zx, false);
//    Plane p4 = Plane(white, vec4(0.0, 555., 0.0, 555.), 555.0, plane_zx, false);
//    Plane p5 = Plane(white, vec4(0.0, 555., 0.0, 555.), 555.0, plane_xy, false);
//    
    //Sphere sph = Sphere(light, vec3(100.,450.,100.0), 40.0); // light
    Sphere sph1 = Sphere(glass, vec3(210., 210., 100.), 35.);
    Sphere spheres[2] = Sphere[](sph, sph1);
...
vec3 trace_ray(Ray r, vec3 background, vec2 seed)
{
    ...
    for(depth = 0; depth < MAX_DEPTH; depth++)
    {
        if(hit(r, 0.001, TMAX, hit_out))
        {
            ...
            {
                r = s_out.out_ray;
                r.origin = r.origin + 0.001*r.direction;
            ...
            pdf_value = (0.5 * ((cosine <= 0.0) ? 0.0 : cosine/pi)) + (0.5 * sphere_pdf(s_out.out_ray));
...
//vec3 denoise(sampler2D img, vec2 uv)
//{
...
//    vec2        samplePixel      = vec2(1.0/1000.,1.0/1000.); 
...
void main()
{
    ...
    //vec2 dims = vec2(1000., 1000.);
    // cornell box
    //Camera cam = make_camera(vec3(278., 278., -800.0), vec3(278.,278.,0.), vec3(0.,1.,0.), 40.0, dims.x / dims.y, 0.0, 10.0); 
